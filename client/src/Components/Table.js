...
// rows
const Rows = (movie, i, admin, onDeleteHandler, downloadImage, progress) => {
  return (
    <tr key={i}>
      <td className={`${Text}`}>
...
// table
function Table({ data, admin, onDeleteHandler, downloadImage, progress }) {
  return (
    <div className="overflow-x-scroll overflow-hidden relative w-full">
      <table className="w-full table-auto border border-border divide-y divide-border">
        ...
        <tbody className="bg-main divide-y divide-gray-800">
          {data.map((movie, i) =>
            Rows(movie, i, admin, onDeleteHandler, downloadImage, progress)
          )}
        </tbody>
      </table>
    </div>
